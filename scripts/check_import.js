  ...
  try {
    ...
    if (countResult[0].total > 0) {
      ...
      const sampleRecords = await sql`
        SELECT id_subsls, kddesa, nmdesa, semester, muatan_kk
        FROM sipw
        LIMIT 5
      `;
      ...
      sampleRecords.forEach((record, index) => {
        console.log(`${index + 1}. ID: ${record.id_subsls}, District: ${record.nmdesa} (${record.kddesa}), Semester: ${record.semester}, Households: ${record.muatan_kk}`);
      });
